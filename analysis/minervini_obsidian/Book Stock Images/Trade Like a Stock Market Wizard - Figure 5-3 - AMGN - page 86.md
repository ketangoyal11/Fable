---
type: book-stock-image
book: "Trade Like a Stock Market Wizard"
figure: "5.3"
page: 86
ticker: "AMGN"
yahoo_symbol: "AMGN"
date_range: "1992-1992"
extraction_confidence: "caption-with-ticker"
generated: "2026-05-17"
private_source_image: true
---

# Figure 5.3 - AMGN - Page 86

## Source Image

Book: [[Trade Like a Stock Market Wizard]]

Caption: Amgen (AMGN) in stage 2: 1992

![](../assets/book_pages_private/trade-like-a-stock-market-wizard/page_086.png)

## Yahoo OHLCV Rebuild

Download status: `OK`

![](../assets/book_stock_charts/trade-like-a-stock-market-wizard/trade-like-a-stock-market-wizard-figure-5-3-amgn-page-86_yahoo_rebuild.png)

CSV: `data/book_stock_images/trade-like-a-stock-market-wizard-figure-5-3-amgn-page-86_ohlcv.csv`

## Pattern Read

Tags: vcp-or-tightening, volume-dry-up, stage-2-leadership

Concepts: [[Pivot and Entry]], [[Relative Strength Leadership]], [[Stage 2 Uptrend]], [[Trend Template]], [[Volatility Contraction Pattern]], [[Volume Dry-Up and Accumulation]]

The useful clue is contraction: the later portion of the window became tighter than the earlier portion. Volume contraction supports the idea that supply was drying up near the tight area.

## Reconciliation Metrics

| Metric | Value |
|---|---:|
| first_close | 2.6562 |
| last_close | 7.375 |
| max_gain_pct | 268.24 |
| max_drawdown_from_period_high_pct | -60.38 |
| first_half_depth_pct | 315.49 |
| second_half_depth_pct | 131.45 |
| tightening | True |
| volume_dryup | True |
| best_trend_template_score | 5/5 |
| latest_trend_template_score | 5/5 |

## Trend Template Checks

- close > 50 DMA
- close > 150 DMA
- close > 200 DMA
- 50 DMA > 150 DMA
- 150 DMA > 200 DMA

## Study Questions

- Does the rebuilt OHLCV chart confirm the same structure shown in the book image?
- Was the stock close to a definable pivot, or already extended?
- Did volume dry up before the move, or was supply still obvious?
- Was this a buy lesson, a sell lesson, or a failure-avoidance lesson?
- What would invalidate the setup if this were being traded live?




<!-- STAGE_LIFECYCLE_START -->
## Stage Lifecycle & Base Concept Analysis
> This section analyzes the FULL LIFECYCLE of the stock around the inferred entry — Stage 1 (Accumulation), Stage 2 (Advance), Stage 3 (Distribution), Stage 4 (Decline) — plus deep base concept analysis, VCP footprint, tight footprint, supply dynamics, and contraction timeline.
- Status: `ok`
- Entry date: `1992-07-29`
- Entry price: `8.2031`
### Stage Lifecycle Overview
| Stage | Present | Start Date | End Date | Duration | Key Signal |
|---|---|---|---:|---|---|
| Stage 1 — Accumulation | ✅ | `1991-11-25` | `1992-11-23` | 252 days | Base: deep-chaotic |
| Stage 2 — Advance | ✅ | `1992-11-23` | `1993-01-18` | 37 days | Max gain: 4.0% |
| Stage 3 — Distribution | ✅ | `1993-05-24` | `1993-06-29` | 25 days | no climax |
| Stage 4 — Decline | ❌ | — | — | — | Not detected |
### Stage 1 — Accumulation / Base Building
- Base type: `deep-chaotic`
- Lowest price in base: `6.1600`
- Volume pattern: `neutral`
### Base Concept Deep-Dive

- Base type: `N/A`
- Base duration: `0 sessions`
- Base depth: `N/A`
- Base high: `N/A`
- Base low: `N/A`
- Resistance touches at base high: `0`
- Support touches at base low: `0`
- Contraction count: `0`
- Contraction quality: `N/A`
- Pivot clarity: `N/A`
- Pivot distance at entry: `N/A`
- Volume dry-up in base: `N/A`
- Volume dry-up ratio: `N/A`
- Tightness at pivot (10d): `N/A`
- Weekly tightness: `N/A`

### VCP Footprint

- VCP present: `False`
- No clear VCP pattern detected in the base.

### Tight Footprint

- 10-session tightness at entry: `5.0%`
- 20-session tightness at entry: `7.0%`
- Weekly tightness: `3.4%`
- ATR20 %: `2.68`
- Tightness progression: `improving`

### Supply Analysis

- Supply label: `neutral`
- Volume dry-up ratio: `0.92`
- Distribution volume detected: `False`
- Accumulation volume detected: `False`

### Concept Tie-Back

- Related concepts: [[Base Concept]], [[Stage 2 Uptrend]], [[Trend Template]], [[Stage 3 Distribution]]
- Lesson: Stage 1 base was deep-chaotic with 58.9% depth. Stage 2 advance lasted 38 sessions with 0 significant pivots.

<!-- STAGE_LIFECYCLE_END -->
<!-- PRE_ENTRY_SENSE_CHECK_START -->

## Pre-Entry Sense Check

> This section analyzes the chart structure PRIOR to the inferred entry. It answers: What did the setup look like in the weeks and months before the trade? Which Minervini concepts were already visible?

- Status: `ok`
- Entry date: `1992-07-29`
- Pre-entry history available: `297 sessions`

### Trend Template Evolution

| Lookback | Date | Score | Assessment |
|---|---|---:|:---|
| 60 days before | 1992-05-04 | 2/7 | 🔴 Not yet Stage 2 |
| 40 days before | 1992-06-02 | 4/7 | 🟡 Transitioning |
| 20 days before | 1992-06-30 | 5/7 | 🟡 Transitioning |

### Pre-Entry Context Window

- Context window (last sessions before entry): `150 sessions`
- Range high: `9.7800`
- Range low: `6.1600`
- Total range depth: `58.9%`
- Contraction phases (rolling 21-bar segments): `28.8% -> 19.4% -> 13.3% -> 24.3% -> 21.6% -> 16.2% -> 16.7%`

### Stage 2 Onset

- First sustained Stage 2 date: `1992-03-11`
- Days in Stage 2 before entry: `97`

### Volume Behavior Before Entry

- Volume dry-up label: `neutral`
- Recent/base volume ratio: `0.92`
- Volume spike dates (2.5x avg) in last 40 days: `1992-07-22`

### Tightness Progression

| Lookback | 10-Session Close Tightness |
|---|---:|
| 40 days before | `11.9%` |
| 20 days before | `12.4%` |
| Final 10 sessions before | `5.0%` |
| Final 3 weekly closes | `3.4%` |

### Moving Average Alignment

- 50/150/200 DMA first aligned (50>150>200): `1992-03-11`

### Shakeouts / Tests Before Entry

- No shakeouts or undercut-recover patterns detected in last 40 sessions before entry.

### 52-Week High Context

| Timing | Distance from 52W High |
|---|---:|
| 60 days before | `N/A` |
| 20 days before | `-22.2%` |
| At entry | `-16.1%` |

### Concept Tie-Back

- Related concepts: [[Stage 2 Uptrend]], [[Trend Template]], [[Relative Strength Leadership]], [[Volatility Contraction Pattern]], [[Pivot and Entry]]
- Lesson: Stage 2 was established 97 days before entry, confirming leadership context. Total pre-entry range was 58.9% — wide range indicating significant prior movement. Volume did not show clear dry-up — supply may still be present.

<!-- PRE_ENTRY_SENSE_CHECK_END -->
<!-- SEPA_REPLICATION_START -->

## SEPA Trade Replication

> Study note: this reconstructs a likely Minervini-style setup area from the real OHLCV window shown by the book timing. It does not claim to know Minervini's private fill, sizing, or unpublished execution.

- Status: `reconstructed-from-real-ohlcv`
- Setup type: `vcp/contraction-study`
- Confidence: `high`
- Timing source: `1992-1992` from the figure caption and rebuilt OHLCV where available.
- Inferred study entry date: `1992-07-29`
- Inferred study entry price: `8.2031`
- Inferred pivot: `8.1875`
- Inferred stop / invalidation: `7.6250`
- Pivot extension at entry: `0.2%`
- Stop distance / risk: `7.6%`
- Trend Template score at entry: `6/7`

### Tightness And Supply
- 3-part pre-entry contraction depth: `22.9% -> 16.2% -> 13.4%`
- Contraction quality: `clear-tightening`
- 10-session close tightness: `5.0%`
- 3-week close tightness: `3.4%`
- Volume dry-up: `neutral`
- Recent/base median volume ratio: `0.92`
- Leadership proxy: 65-day return 27.4% and 126-day return -2.8%

### Post-Entry Reality Check
- Max gain after 20 sessions: `2.9%`
- Max gain after 60 sessions: `6.3%`
- Max gain after 120 sessions: `18.9%`
- Worst drawdown after 20 sessions: `-7.0%`
- Inferred stop failed within 20 sessions: `False`
- Pivot broadly respected within 20 sessions: `False`

### Concept Tie-Back

- Related concepts: [[Risk First]], [[Volatility Contraction Pattern]], [[Volume Dry-Up and Accumulation]], [[Pivot and Entry]], [[Trend Template]], [[Stage 2 Uptrend]], [[Relative Strength Leadership]]
- Lesson: The reconstructed data suggests price was becoming more controllable before the inferred entry; risk was close enough for a clean SEPA-style test.

<!-- SEPA_REPLICATION_END -->
