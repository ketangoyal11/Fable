#!/usr/bin/env python3
"""
Staircase Strategy Weekly Backtest Engine V3 — Yahoo Finance Universe
======================================================================
Runs the V3 strategy (valid days REMOVED, point-in-time TF checks only)
on the full 731-symbol yfinance dataset.

Data source: data/yfinance_universe/history/
"""
...
HISTORY_DIR = ROOT / "data" / "yfinance_universe" / "history"
OUTPUT_DIR = ROOT / "analysis" / "staircase_dhan"
OUTPUT_EXCEL = OUTPUT_DIR / "staircase_strategy_weekly_backtest_v3_yfinance_universe.xlsx"
OUTPUT_JSON = OUTPUT_DIR / "staircase_strategy_weekly_backtest_v3_yfinance_universe.json"
...
def get_symbols_with_history() -> List[str]:
    """Get all symbols from yfinance history directory."""
    files = sorted(HISTORY_DIR.glob("*.csv"))
    return [f.stem for f in files]


def load_yfinance_history(symbol: str) -> Optional[pd.DataFrame]:
    """Load daily OHLCV from yfinance history CSV."""
...
def main():
    ...
    print("=" * 70)
    print("STAIRCASE STRATEGY WEEKLY BACKTEST ENGINE V3 — YAHOO FINANCE UNIVERSE")
    print("=" * 70)
    print(f"Data Source: Yahoo Finance ({HISTORY_DIR})")
    ...
    for idx, symbol in enumerate(symbols, 1):
        try:
            daily_df = load_yfinance_history(symbol)
            if daily_df is None:
            ...
            if idx % 100 == 0 or idx == len(symbols):
                print(f"  ... {idx}/{len(symbols)} processed (total positions: {len(all_positions)})")
