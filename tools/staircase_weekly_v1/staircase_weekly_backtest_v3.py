#!/usr/bin/env python3
"""
Staircase Strategy Weekly Backtest Engine V3
=============================================
ALIGNED with Pine Script: staircase_SMA - MTF Gated + TF Darvas Entry Gate

KEY CHANGES FROM V2:
- Data source: yfinance PRIMARY (matches TV data closely)
- Valid days logic: REMOVED (point-in-time TF checks only)
- Settings matched to user's TV input panel exactly
- Symbol mapping for yfinance (APOLLOMICRO -> APOLLO.NS)
"""
...
OUTPUT_EXCEL = OUTPUT_DIR / "staircase_strategy_weekly_backtest_v3.xlsx"
...
TF1_SHORT = True
TF1_LONG = False
TF1_DARVAS = False       # User unsure; set False, make configurable
...
TF2_SHORT = True
TF2_LONG = True
TF2_DARVAS = False       # Default from Pine Script
...
# yfinance symbol mapping for NSE stocks
YF_SYMBOL_MAP = {
    "APOLLOMICRO": "APOLLO.NS",
}

# =============================================================================
# DATA LOADING (yfinance PRIMARY, Dhan fallback)
# =============================================================================

def load_yfinance_data(symbol: str) -> Optional[pd.DataFrame]:
...
def load_dhan_data(symbol: str) -> Optional[pd.DataFrame]:
    """Load daily data from Dhan CSVs."""
...
def load_daily_data(symbol: str) -> pd.DataFrame:
    """Load daily data: yfinance primary, Dhan fallback."""
    df = load_yfinance_data(symbol)
    if df is not None and len(df) >= 50:
        print(f"  [{symbol}] Using yfinance data ({len(df)} rows)")
        return df.sort_values("date").reset_index(drop=True)

    df = load_dhan_data(symbol)
    if df is not None and len(df) >= 50:
        print(f"  [{symbol}] Using Dhan data ({len(df)} rows)")
        return df.sort_values("date").reset_index(drop=True)

    # Final fallback: try .BO suffix on yfinance
    try:
        ...
        if len(hist) > 50:
            ...
            hist["date"] = pd.to_datetime(hist["date"]).dt.tz_localize(None)
            df = hist[["date", "open", "high", "low", "close", "volume"]].copy()
            print(f"  [{symbol}] Using yfinance (.BO) data ({len(df)} rows)")
            return df.sort_values("date").reset_index(drop=True)
    except Exception:
        pass
...
def main():
    print("=" * 70)
    print("STAIRCASE STRATEGY WEEKLY BACKTEST ENGINE V3")
    print("=" * 70)
    print(f"Data Source: yfinance PRIMARY, Dhan fallback")
