"""Price adapter - pulls OHLCV from Yahoo Finance, computes all Staircase indicators.
Accepts optional strategy dict so all thresholds are tunable from strategy.yaml."""
import asyncio
import aiohttp
import numpy as np
...
async def fetch(asset="XAU/USDT", strategy=None):
    if strategy is None:
        strategy = {}
    ...
    async with aiohttp.ClientSession() as session:
        for attempt in range(3):
            ...
            raise RuntimeError(f"Yahoo Finance rate limited after 3 retries")
            ...
            sma_xu = False
            if i > 0:
                ema_xu = ema_fast[i - 1] >= ema_slow[i - 1] and ema_fast[i] < ema_slow[i]
                sma_xu = sma50[i - 1] >= sma100[i - 1] and sma50[i] < sma100[i]
