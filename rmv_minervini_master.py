...
EMA20_LEN = 20 * TF_MULT
EMA21_LEN = 21 * TF_MULT
EMA50_LEN = 50 * TF_MULT
EMA150_LEN = 150 * TF_MULT
EMA200_LEN = 200 * TF_MULT

# Volume / Breakout thresholds
VR_BREAKOUT = 1.3
VR_ENTRY_MIN = 0.90
VR_ENTRY_MAX = 1.30
BREAKOUT_VOL_LOOKBACK_MIN = 4
BREAKOUT_VOL_LOOKBACK_MAX = 6
VR_CHEAT_MAX = 1.5
VR_CLIMAX = 2.0
MIN_CHG_PCT = 2.0
CLIMAX_CHG_PCT = 3.0
...
# Minervini Quality Filters
MAX_PIVOT_DISTANCE_PCT = 5.0  # Max % above pivot for valid cheat entry
...
def compute_ema(values, period):
    ...
    for i in range(len(values)):
        if i == 0:
            ...
            # Warm up with a simple average until enough data exists.
...
def compute_vr(volume, avg_vol_period):
    """Volume Ratio: current volume / EMA of volume over period."""
    vol_ema = compute_ema(volume, avg_vol_period)
    vr = []
    for i in range(len(volume)):
        if vol_ema[i] is not None and vol_ema[i] > 0:
            vr.append(volume[i] / vol_ema[i])
        else:
            vr.append(1.0)
    return vr


def find_swing_highs(highs, window=3, min_bars_between=5, min_significance_pct=1.0):
    """
    Find local maxima (swing highs) in the high series.
    A swing high is a bar whose high is the highest of [i-window, i+window].
    Filters out noise by requiring minimum significance vs surrounding closes.
    """
    n = len(highs)
    swing_highs = []
    for i in range(window, n - window):
        if highs[i] == max(highs[i - window:i + window + 1]):
            # Significance: this high must be meaningfully above neighbors
            surrounding_max = max(highs[i - window:i] + highs[i + 1:i + window + 1]) if window > 0 else 0
            if surrounding_max > 0:
                significance = (highs[i] - surrounding_max) / surrounding_max * 100
                if significance < min_significance_pct:
                    continue
            if not swing_highs or (i - swing_highs[-1][0]) >= min_bars_between:
                swing_highs.append((i, highs[i]))
    return swing_highs


def trendline_value_at(p1, p2, bar_idx):
    """Project a trendline defined by two (bar, price) points to a given bar."""
    b1, pr1 = p1
    b2, pr2 = p2
    if b2 == b1:
        return pr1
    slope = (pr2 - pr1) / (b2 - b1)
    return pr1 + slope * (bar_idx - b1)


def build_anchor_points(highs, swing_highs, companion_lookback=5, companion_tolerance_pct=3.0):
    """
    Build an expanded list of trendline anchor points.
    Includes strict swing highs PLUS 'companion highs' — nearby bars that tested
    the same resistance level but aren't strict local maxima (e.g. Mar 17 near
    the Mar 18 swing high in MU 2026).
    """
    anchors = list(swing_highs)  # (bar, high) tuples
    anchor_bars = {b for b, _ in anchors}
    
    for swing_bar, swing_high in swing_highs:
        # Look at bars just before the swing high
        for k in range(max(0, swing_bar - companion_lookback), swing_bar):
            if k in anchor_bars:
                continue
            high_k = highs[k]
            if high_k <= 0:
                continue
            # Companion = within tolerance of the swing high (same resistance zone)
            if abs(high_k - swing_high) / swing_high * 100 <= companion_tolerance_pct:
                # Must be a locally significant high EXCLUDING the strict swing high itself
                local_window = 1
                neighborhood = highs[max(0, k - local_window):min(len(highs), k + local_window + 1)]
                # Exclude the strict swing high bar if it's in the neighborhood
                filtered = [h for idx, h in enumerate(
                    highs[max(0, k - local_window):min(len(highs), k + local_window + 1)],
                    start=max(0, k - local_window))
                    if idx != swing_bar]
                if not filtered:
                    continue
                if high_k == max(filtered):
                    anchors.append((k, high_k))
                    anchor_bars.add(k)
    
    # Sort by bar index and remove duplicates
    anchors = sorted(set(anchors), key=lambda x: x[0])
    return anchors


def detect_trendline_breakouts(i, highs, closes, vr_vals, rmv_vals, stage2, chg_pct,
                                swing_highs, lookback=100,
                                min_consolidation_tight_bars=3,
                                min_consolidation_lookback=10,
                                min_vr=0.85, min_chg_pct=1.5,
                                breakout_buffer_pct=0.25,
                                companion_lookback=5,
                                companion_tolerance_pct=3.0,
                                min_close_range_pct=4.0,
                                min_close_range_lookback=5):
    """
    Detect breakout above a trendline formed by recent swing highs and companion
    resistance points. Returns (is_breakout, trendline_value, anchor_points) or
    (False, None, None).
    """
    if i < 10 or not stage2[i]:
        return False, None, None
    
    anchors = build_anchor_points(highs, swing_highs,
                                   companion_lookback=companion_lookback,
                                   companion_tolerance_pct=companion_tolerance_pct)
    recent = [(b, h) for b, h in anchors if (i - b) <= lookback and b < i - 5]
    if len(recent) < 2:
        return False, None, None
    
    # Check consolidation: either RMV-tight bars OR tight closing-price range
    tight_count = sum(1 for k in range(max(0, i - min_consolidation_lookback), i)
                      if rmv_vals[k] <= RMV_TIGHT_THRESHOLD)
    recent_closes = closes[max(0, i - min_close_range_lookback):i]
    close_range_pct = 100.0
    if len(recent_closes) >= 3 and max(recent_closes) > 0:
        close_range_pct = (max(recent_closes) - min(recent_closes)) / max(recent_closes) * 100
    
    has_consolidation = (tight_count >= min_consolidation_tight_bars or
                         close_range_pct <= min_close_range_pct)
    if not has_consolidation:
        return False, None, None
    
    # Try trendlines using pairs of recent anchor points.
    # Collect all valid pairs and return the one with the lowest trendline value
    # (best entry / earliest breakout) that price clears.
    valid_breakouts = []
    for idx_new in range(len(recent) - 1, max(0, len(recent) - 4), -1):
        p_new = recent[idx_new]
        for idx_old in range(idx_new):
            p_old = recent[idx_old]
            if p_new[0] == p_old[0]:
                continue
            # Require meaningful separation in time and price
            if p_new[0] - p_old[0] < 10:
                continue
            tl_val = trendline_value_at(p_old, p_new, i)
            # Trendline must be at or above the newer anchor's level
            # (flat/ascending trendlines only)
            if tl_val < p_new[1] * 0.98:
                continue
            # Price must close above trendline
            threshold = tl_val * (1 + breakout_buffer_pct / 100)
            if closes[i] <= threshold:
                continue
            # Volume and momentum checks (relaxed vs horizontal breakout)
            if vr_vals[i] < min_vr:
                continue
            if chg_pct[i] < min_chg_pct:
                continue
            # Require price to have been near the trendline recently
            # (not a massive gap above it)
            prox_window = range(max(0, i - 8), i)
            if len(list(prox_window)) > 0:
                proximity = min(abs(closes[k] - tl_val) / tl_val * 100 for k in prox_window)
                if proximity > 10:
                    continue
            valid_breakouts.append((tl_val, p_old, p_new))
    
    if valid_breakouts:
        # Pick the lowest trendline value = best entry
        valid_breakouts.sort(key=lambda x: x[0])
        best_tl, best_old, best_new = valid_breakouts[0]
        return True, best_tl, (best_old, best_new)
...
def analyze(data, ticker="STOCK"):
    ...
    ema20 = compute_ema(closes, EMA20_LEN)
    ema21 = compute_ema(closes, EMA21_LEN)
    ema50 = compute_ema(closes, EMA50_LEN)
    ema150 = compute_ema(closes, EMA150_LEN)
    ...
    # -- EMA200 Rising (compare to 20 bars ago) --
    ema200_rising = [False] * n
    for i in range(20 * TF_MULT, n):
        if ema200[i] is not None and ema200[i - 20 * TF_MULT] is not None:
            ema200_rising[i] = ema200[i] > ema200[i - 20 * TF_MULT]

    # -- ATH (All-Time High = 52wk high) --
    ath = wk52_high

    # -- Stage Detection (EMA-only, loosened short-term Stage 2) --
    stage2 = [False] * n
    stage1 = [False] * n
    stage3 = [False] * n
    stage4 = [False] * n
    for i in range(n):
        if ema10[i] is not None and ema20[i] is not None and ema50[i] is not None and \
                ema150[i] is not None and ema200[i] is not None:
            stage2[i] = (closes[i] > ema50[i] and ema10[i] > ema50[i] and
                         ema20[i] > ema50[i] and closes[i] > ema200[i])
            stage1[i] = (closes[i] < ema200[i] and closes[i] < ema150[i] and
                         ema150[i] < ema200[i])
            stage3[i] = (closes[i] < ema50[i] and closes[i] > ema200[i])
            stage4[i] = (closes[i] < ema50[i] and ema50[i] < ema150[i] and
                         ema50[i] < ema200[i] and closes[i] < ema200[i] * 0.95)
    ...
    for i in range(n):
        ...
        if ema10[i] is not None and ema20[i] is not None and ema50[i] is not None and ema200[i] is not None:
            trend_template[i] = (closes[i] > ema50[i] and ema10[i] > ema50[i] and
                                 ema20[i] > ema50[i] and closes[i] > ema200[i] and
                                 near_52w_high and
                                 far_from_52w_low and ema200_rising[i])
    ...
    # -- Swing highs for trendline breakout detection --
    swing_highs = find_swing_highs(highs, window=3, min_bars_between=5, min_significance_pct=1.0)
    ...
    for i in range(n):
        ...
        bar = {
            ...
            'short_term_trend_ok': False,
            'strict_buy_context': False,
        ...
        # =======================================================================
        recent_vol_confirm = False
        if i >= BREAKOUT_VOL_LOOKBACK_MIN:
            vol_start = max(0, i - BREAKOUT_VOL_LOOKBACK_MAX)
            recent_vol_confirm = volumes[i] > max(volumes[vol_start:i])

        breakout_vr_confirm = VR_ENTRY_MIN <= vr_vals[i] <= VR_ENTRY_MAX
        bar['breakout_volume_confirmed'] = breakout_vr_confirm or recent_vol_confirm
        bar['generic_breakout'] = bar['breakout_volume_confirmed'] and chg_pct[i] >= MIN_CHG_PCT

        if ema10[i] is not None and ema20[i] is not None and ema50[i] is not None:
            bar['short_term_trend_ok'] = ema10[i] > ema50[i] and ema20[i] > ema50[i]

        bar['strict_buy_context'] = (stage2[i] and trend_template[i] and
                                     (closes[i] > ema200[i] if ema200[i] else False) and
                                     bar['short_term_trend_ok'])
        
        # =======================================================================
        # PATTERN: EXTENDED
        # =======================================================================
        if ema50[i] is not None and ema50[i] > 0:
            bar['extended'] = ((closes[i] - ema50[i]) / ema50[i]) * 100 > 30.0
        
        # =======================================================================
        # PATTERN: BRK-STOP
        # =======================================================================
        vol_ema_arr = compute_ema(volumes, VR_LENGTH)
        if ema20[i] is not None and vol_ema_arr[i] is not None:
            bar['brk_stop'] = closes[i] < ema20[i] and volumes[i] > vol_ema_arr[i] * 1.3
        ...
        if ema10[i] is not None and ema10[i] > 0:
            ...
            bar['scale_in'] = (closes[i] > ema10[i] and ema10_dist <= 2.5 and
                               (closes[i] > ema50[i] if ema50[i] else False) and
                               stage2[i] and bar['short_term_trend_ok'] and
                               vr_vals[i] >= 1.0 and chg_pct[i] >= 1.0)
        ...
        #   Breakout above a trendline formed by recent swing highs, especially
        #   after a tight consolidation (flag). Relaxed volume vs horizontal BO.
        # =======================================================================
        is_tl_break, tl_value, tl_anchors = detect_trendline_breakouts(
            i, highs, closes, vr_vals, rmv_vals, stage2, chg_pct, swing_highs
        )
        bar['trendline_break'] = is_tl_break
        bar['trendline_value'] = tl_value
        ...
        for p in pivots:
            ...
            is_vol_break = bar['breakout_volume_confirmed']
            is_pct_break = chg_pct[i] >= MIN_CHG_PCT
            is_true_break = is_price_break and is_vol_break and is_pct_break
            strict_buy_context = bar['strict_buy_context']
            
                        # CHEAT ENTRY
            is_cheat_entry = is_price_break and is_vol_break and chg_pct[i] >= MIN_CHG_PCT and pivot_dist_above <= MAX_PIVOT_DISTANCE_PCT
            ...
            is_low_cheat_now = (not broken and not failed and closes[i] < p_price and
                                ...
                                (closes[i] > ema50[i] if ema50[i] else False) and
                                bar['strict_buy_context'] and tight_now[i])
            if is_low_cheat_now:
                bar['low_cheat'] = True
                bar['pivot_dist_below'] = pivot_dist_below
            ...
            if is_true_break and strict_buy_context and not broken and not failed:
                if is_vcp_base[i] and p_days >= MIN_BASE_WEEKS * 5 and p_depth <= MAX_BASE_DEPTH:
        ...
        # =======================================================================
        if bar['spring'] and bar['strict_buy_context'] and not bar['entry_signal']:
        ...
        if (bar['trendline_break'] and bar['strict_buy_context'] and
                bar['breakout_volume_confirmed'] and not bar['entry_signal']):
            bar['break_type'] = 'TRENDLINE-B/O'
            bar['break_color'] = 'orange'
            bar['entry_signal'] = True
            # Set bracket stops off the trendline value if no pivot stop exists
            if bar['bracket_stop_1'] is None and bar['trendline_value'] is not None:
                tl = bar['trendline_value']
                bar['bracket_stop_1'] = tl * (1 - STOP1_PCT / 100)
                bar['bracket_stop_2'] = tl * (1 - STOP2_PCT / 100)
                bar['pivot_price'] = tl
...
def print_results(results, pivots, ticker):
    ...
    tl_count = sum(1 for r in results if r['trendline_break'])
    ...
    print(f"  {'TRENDLINE-BREAK':<30} {tl_count:<8} {tl_count/max(n,1)*100:>7.1f}%")
    ...
    for i, r in enumerate(results):
        ...
        # trendline_break entry is already captured via break_type above
...
def print_comparison_report():
    ...
    rows = [
        ...
        ("BREAKOUT (VR .90-1.30 + 4-6bar vol)", "V STRICT", "V", "V", "V volume confirm"),
        ("VCP-B/O (breakout)", "V EXACT", "~ diff halves", "~ 20d range", "-"),
        ("CHEAT (strict breakout volume)", "V STRICT", "~ VR<1.5", "~ VR<1.5", "~ 3-C CHEAT"),
